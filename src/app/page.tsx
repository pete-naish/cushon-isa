...
const ANNUAL_ISA_LIMIT = 20000;
const MIN_LUMP_SUM = 100;
const MAX_SELECTED_FUNDS: number = 2; // implement logic to limit how many can be selected. 1 would mean selection changes every click. More than one, clicking will toggle selected. Show next button when at least one is selected.

function toggleItem<T>(item: T) {
...
// forecast for 1 year by default

const categories = ["Ethical", "CushonMix", "Equities", "Bonds"] as const;
...
export default function Page() {
  ...
  // rename
  const handleFundDepositChange = (id: string, value: string) => {
  ...
  const remainingAllowance = (ANNUAL_ISA_LIMIT - selectedFundTotal).toFixed(2);
          ...
          return (
            <div key={fund.id}>
              ...
              <input
                ...
                value={fund?.total || ""}
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  handleFundDepositChange(fund.id, e.target.value)
                }
